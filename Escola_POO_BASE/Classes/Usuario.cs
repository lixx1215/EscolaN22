    ...
    public class Usuario
    {
        ...
        public static Usuario RealizarLogin(string email, string senha)
        {
            ...
            try
            {
                ...
                if (cn.dr.HasRows)
                {
                    //Achou os dadols do usuário de acordo com o E-mail pesquisado
                    while (cn.dr.Read())
                    {
                        ...
                        usuario.NivelAcesso = Convert.ToInt32((int)cn.dr[6]);
        ...
        {
            string query = string.Format($"UPDATE Professor SET Senha = '{Crypto.Sha256(novaSenha)}' WHERE Id = {Id}");
            Conexao cn = new Conexao(query);
